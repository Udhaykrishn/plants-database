...
// ── Field config ─────────────────────────────────────────────────────────────
const ALL_FIELDS: { key: string; label: string; width: string; required?: boolean }[] = [
    ...
    { key: 'icon_url', label: 'Icon URL', width: '160px' },
    { key: 'image_url', label: 'Image URL', width: '160px' },
];
                                ...
                                <tbody>
                                    {rows.map((row, rowIdx) => {
                                        ...
                                        return (
                                            ...
                                            >
                                                {/* Row num */}
                                                <td className="text-center text-[10px] text-muted-foreground/40 tabular-nums border-r border-b border-border px-1 sticky left-0 z-10 bg-inherit">
                                                    {rowIdx + 1}
                                                </td>
                                                {ALL_FIELDS.map(f => (
                                                    <td key={f.key} className="border-r border-b border-border p-0">
                                                        <textarea
                                                            value={row[f.key] ?? ''}
